...
class DevicesConf(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self["actions"] = ActionMap(
            ...
            {"green": self.saveMypoints, "red": self.close, "back": self.close},
        )
        self.updateList()
    ...
    def saveMypoints(self):
        ...
        for x in self["config"].list:
            ...
            self.Console.ePopen("umount " + self.device)
            self.Console.ePopen(
                "/sbin/blkid | grep " + self.device + " && opkg list-installed ntfs-3g",
                self.add_fstab,
                [self.device, self.mountp],
            )
    ...
    def delay(self, val):
        if (
            fileExists("/etc/init.d/volatile-media.sh")
            and getBoxHostName() == "vusolo2"
        ):
...
class SetDiskLabel(Screen):
    ...
    def wlacz(self):
        self.session.openWithCallback(
            self.wlaczyes,
            MessageBox,
            _("Set label on %s?") % str(self["devlist"].getCurrent()),
    ...
    def dellabel(self):
        self.session.openWithCallback(
            self.delabelyes,
            MessageBox,
            _("Delete label from %s?") % str(self["devlist"].getCurrent()),
            MessageBox.TYPE_YESNO,
            default=False,
        )
