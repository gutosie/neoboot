...
class CreateSwap(Screen):
    ...
    skin = """<screen name="Swap" title="NeoBoot - Create Swap " position="center,center" size="892,198" flags="wfNoBorder">
        ...
        <widget name="key_green" position="75,140" zPosition="2" size="315,45" font="baslk;30" halign="left" valign="center" backgroundColor="red" transparent="1" foregroundColor="red" />
        <widget name="key_red" position="490,140" zPosition="2" size="315,45" font="baslk;30" halign="left" valign="center" backgroundColor="green" transparent="1" foregroundColor="green" />
        </screen>"""
    ...
    def RemoveSwap(self):
            if os.path.exists('/media/hdd/swapfile'):
                        ...
                        cmd1 = '/sbin/swapoff -a; rm -rf /media/hdd/swapfile '
                        self.session.open(Console, _('NeoBoot....'), [cmd0,
                         cmd1,])
                        self.close()
            elif os.path.exists('/media/usb/swapfile'):
                        cmd0 = "echo -e '\n\n%s '" % _('Remove swap, please wait...')
                        cmd1 = '/sbin/swapoff -a; rm -rf /media/usb/swapfile'
                        self.session.open(Console, _('NeoBoot....'), [cmd0,
                         cmd1,])
                        self.close()
            elif os.path.exists('/swapfile'):
                        cmd0 = "echo -e '\n\n%s '" % _('Remove swap, please wait...')
                        cmd1 = '/sbin/swapoff -a; rm -rf /swapfile'
                        self.session.open(Console, _('NeoBoot....'), [cmd0,
                         cmd1,])
                        self.close()
            else:
