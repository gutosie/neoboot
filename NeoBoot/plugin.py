...
class MyUpgrade(Screen):
    ...
    def CRASHlogNeo(self):
        showlog = (
            'echo "\nCRARSH LOG-neoboot startup error!"  >> '
            + getNeoLocation()
            + "ImageBoot/neoboot.log; cat "
            + getNeoLocation()
...
class NeoBootInstallation(Screen):
    if isFHD():
        from Plugins.Extensions.NeoBoot.neoskins.default import (
    ...
    def datadrive(self):
        try:
            ...
            self.session.open(
                Console, _("    NeoBot - Available media:"), [message, cmd]
            )
        except Exception as e:
            loggscrash = time.localtime(time.time())
            LogCrashGS(
    ...
    def checkReadWriteDir(self, configele):
        ...
        if candidates:
            ...
            if (
                Harddisk.findMountPoint(os.path.realpath(configele)) + "/" in locations
                or Harddisk.findMountPoint(os.path.realpath(configele)) in locations
            ):
    ...
    def devices(self):
        check = False
        if (
            not fileExists("/usr/lib/python2.7")
            or fileExists("/usr/lib/python3.8")
            or fileExists("/usr/lib/python3.9")
            or fileExists("/usr/lib/python3.10")
            or fileExists("/usr/lib/python3.11")
            or fileExists("/usr/lib/python3.12")
            or fileExists("/usr/lib/python3.13")
            or fileExists("/tmp/.upneo")
        ):
            if fileExists("/.multinfo"):
                ...
                from Plugins.Extensions.NeoBoot.files.tools import (
                    InitializationFormattingDisk,
                )
            ...
            message += _(
                "Do you want to run the manager to mount the drives correctly ?\n"
            )
            ybox = self.session.openWithCallback(
    ...
    def first_installation(self):
        ...
        if fileExists("/proc/mounts"):
            with open("/proc/mounts", "r") as f:
                for line in f.readlines():
                    if (
                        line.find(" ext")
                        and line.find("/media/hdd")
                        or line.find("/media/usb") == -1
                        and (
                            line.find("ext4") != -1
                            or line.find("ext3") != -1
                            or line.find("ext2") != -1
        ...
        if not check:
            ...
            if self.checkReadWriteDir(self.mysel):
                message = (
                    _("Do You really want to install NeoBoot in:\n ")
                    + self.mysel
                    + "  ?"
                )
                ybox = self.session.openWithCallback(
    ...
    def first_installationNeoBoot(self):
        ...
        if not fileExists("/usr/lib/periodon/.accessdate") or not fileExists(
            "/etc/name"
        ):
        ...
        if not os.path.isfile("/etc/name"):
            if (
                ...
                or getBoxHostName() == "dm520"
                or getBoxHostName() == "dm525"
                or getBoxHostName() == "dm800"
                or getBoxHostName() == "dm800se"
                or getBoxHostName() == "dm8000"
            ):
        ...
        if getCPUtype() == "ARMv7":
            if getBoxHostName() == "vuduo4k" and getBoxHostName() != "ustym4kpro":
                ...
                getBoxHostName() == "lunix4k"
                and getCPUSoC() == "72604"
                and getBoxHostName() != "vuzero4k"
            ):
                os.system(
                    "cp -af " +
                    LinkNeoBoot +
                ...
                getBoxHostName() == "revo4k"
                and getCPUSoC() == "7252S"
                and getBoxHostName() != "vuuno4k"
            ):
                os.system(
                    "cp -af " +
                    LinkNeoBoot +
                ...
                getBoxHostName() == "osmio4k"
                or getBoxHostName() == "osmio4kplus"
                or getBoxHostName() == "osmini4k"
            ):
                os.system(
                    "mv " +
                    LinkNeoBoot +
            ...
            if fileExists(
                ...
                + getBoxHostName()
                + ".vmlinux.gz"
            ) and not os.path.isfile("/etc/name"):
                os.system(
                    "rm -r "
                    + getNeoLocation()
                    + "ImagesUpload/.kernel/"
            ...
            if (
                ...
                os.system(
                    ...
                    + getNeoLocation()
                    + "ImagesUpload/.kernel/zImage.%s.ipk" % getBoxVuModel()
                )
        ...
        if (
            fileExists("" + LinkNeoBoot + "/ubi_reader_mips")
            or fileExists("" + LinkNeoBoot + "/ubi_reader_arm")
            and fileExists("" + LinkNeoBoot + "/ubi_reader")
        ):
        ...
        if getCPUtype() == "ARMv7":
            os.system(
                "cd "
                + LinkNeoBoot
                + "/;chmod 755 ./files/findsk.sh; mv ./bin/fbcleararm ./bin/fbclear; chmod 755 ./bin/fbclear; rm -f ./bin/nandwrite; rm -f ./bin/fbclearmips; mv ./ubi_reader_arm ./ubi_reader; rm -r ./ubi_reader_mips; rm ./bin/neoinitmips; rm ./bin/neoinitmipsvu; rm -r ./bin/nanddump_mips; rm ./bin/nfidump; rm ./bin/neobmmips; mv ./bin/neobmarm ./bin/neobm; rm ./bin/fontforneoboot.ttf; rm ./bin/libpngneo; rm ./bin/dminit; cd"
            )
        elif getCPUtype() == "MIPS":
            os.system(
                "cd "
                + LinkNeoBoot
                + "/;rm -f ./files/findsk.sh; mv ./bin/fbclearmips ./bin/fbclear; chmod 755 ./bin/fbclear; rm -f ./bin/fbcleararm; mv ./ubi_reader_mips ./ubi_reader; rm -r ./ubi_reader_arm; rm -f /bin/neoinitarm; rm -f /bin/neoinitarmvu; rm -r ./bin/nanddump_arm; rm -f /bin/neoinitarmvuDuo4k; rm -f ./bin/neobmarm"
            )
...
class NeoBootImageChoose(Screen):
    ...
    def __init__(self, session):
        ...
        elif getCheckActivateVip() == getBoxMacAddres() and fileExists(
            "/usr/lib/periodon/.kodn"
        ):
            self["label14"] = Label(_("VIP-on  NeoBoot: "))
        else:
        ...
        if (
            os.path.exists("/media/hdd/swapfile")
            or os.path.exists("/media/usb/swapfile")
            or os.path.exists("/swapfile")
        ):
            system("swapon -a")
        ...
        if yesno:
            if checkInternet():
                ...
                if fileExists("/tmp/img.sh"):
                    cmd2 = (
                        "chmod -R +x /tmp/img.sh; /tmp/img.sh; sleep 2; rm /tmp/img.sh"
                    )
                    system(cmd2)
                if not fileExists(
                    "/usr/lib/enigma2/python/Plugins/Extensions/ImageDownloader/plugin.py"
                ) and fileExists("/usr/bin/curl"):
                    os.system(
    ...
    def ReinstallNeoBoot(self):
        INSTALLbox = self.session.openWithCallback(
            self.reinstallboot,
            MessageBox,
            _("Select Yes to reinstall the neoboot.\n     NEOBOOT."),
            MessageBox.TYPE_YESNO,
        )
        ...
        if answer is True:
            try:
                ...
                cmd1 = (
                    "cd " +
                    LinkNeoBoot +
                    '/; rm ./bin/install; rm ./.location; rm ./files/mountpoint.sh; rm ./files/neo.sh; sleep 5; PATH=/sbin:/bin:/usr/sbin:/usr/bin; echo -n "Restarting E2... "; init 4; sleep 1; init 3 ')
            except BaseException:
                False
            ...
            try:
                self.session.open(
                    MessageBox, _("Resignation."), MessageBox.TYPE_INFO, 4
                )
                self.close()
            except BaseException:
                False
    ...
    def close_exit2(self):
        ...
        if (
            not fileExists("/tmp/.finishdate")
            or not fileExists("/tmp/.nkod")
            or fileExists("/.multinfo")
        ):
            if checkInternet():
                pass
            else:
                ...
                if not fileExists("/usr/lib/periodon"):
                    ...
                    if getButtonPin() == "pinok":
                        os.system(
                            ...
                            UPDATEDATE)
                        if fileExists("/usr/lib/periodon/.accessdate") and fileExists(
                            "/usr/lib/periodon/.kodn"
                        ):
                            restartbox = self.session.openWithCallback(
                                self.GUIRestart,
    ...
    def touch0(self):
        if fileExists("/usr/lib/periodon/.kodn"):
            if not fileExists("/tmp/gut3"):
                ...
                system(
                    "rm -f /tmp/gut*; rm -f /usr/lib/periodon/.kodn; rm -f /usr/lib/periodon/.accessdate; rm -f /tmp/.finishdate; rm -f /tmp/.nkod"
                )
                restartbox = self.session.openWithCallback(
                    self.GUIRestart,
                    MessageBox,
    ...
    def neoboot_update(self):
        if checkInternet():
            if fileExists("/.multinfo"):
                ...
                message = _("\n\n\n")
                message += _(
                    "WARNING !: The update brings with it the risk of errors.\n"
                )
                message += _(
                    "Before upgrading it is recommended that you make a backup NeoBoot.\n"
                )
    ...
    def updateList(self):
        ...
        self.list = [
            fil
            for fil in myimages
            if os.path.isdir(os.path.join(mypath, fil)) and fil != "Flash"
        ]  # Ignore the "Flash" image
    ...
    def RemoveIMAGE(self, yesno):
        if yesno:
            ...
            if fileExists(
                "%sImageBoot/" % getNeoLocation()
                + self.mysel
                + "/usr/lib/periodon/.activatedmac"
            ):
                cmd2 = (
                    "chattr -i %sImageBoot/" % getNeoLocation()
                    + self.mysel
                    + "/usr/lib/periodon/.activatedmac"
                )
            if fileExists(
            ...
            if fileExists(
                ...
                cmd4 = (
                    "chattr -i %sImageBoot > /dev/null 2>&1/" %
                    getNeoLocation() +
                    self.mysel +
                    "" +
                    LinkNeoBoot +
...
def checkimage():
    mycheck = False
    if (
        not fileExists("/proc/stb/info")
        or not fileExists("" + LinkNeoBoot + "/neoskins/neo/neo_skin.py")
        or not fileExists("" + LinkNeoBoot + "/bin/utilsbh")
        or not fileExists("" + LinkNeoBoot + "/stbinfo.cfg")
    ):
        mycheck = False
    else:
        mycheck = True
...
def main(session, **kwargs):
    ...
    if vip == 1:
            ...
            if checkInternet():
                ...
                if fileExists("/tmp/.nkod"):
                    ...
                    if not fileExists("/tmp/ver.txt"):
                        if fileExists("/usr/bin/wget"):
                            os.system(
                                "cd /tmp; wget --no-check-certificate https://raw.githubusercontent.com/gutosie/neoboot/master/ver.txt; wget --no-check-certificate https://raw.githubusercontent.com/gutosie/neoscript/master/.neouser; cd /"
                            )
                    if not fileExists("/tmp/ver.txt"):
                        if fileExists("/usr/bin/fullwget"):
                            os.system(
                                "cd /tmp; fullwget --no-check-certificate https://raw.githubusercontent.com/gutosie/neoboot/master/ver.txt; fullwget --no-check-certificate https://raw.githubusercontent.com/gutosie/neoscript/master/.neouser; cd /"
                            )
        ...
        if fileExists("" + LinkNeoBoot + "/.location") and fileExists(
            "%sImageBoot/.neonextboot" % getNeoLocation()
        ):
            f2 = open("%sImageBoot/.neonextboot" % getNeoLocation(), "r")
            mypath2 = f2.readline().strip()
            f2.close()
            if mypath2 != "Flash" or mypath2 == "Flash" and checkimage():
                if fileExists("/.multinfo") or fileExists(
                    "/usr/lib/periodon/.fullaccess"
                ):
...
def menu(menuid, **kwargs):
    if menuid == "mainmenu":
        return [(_("NeoBOOT"), main, "neo_boot", 1)]
    return []
